...
import { active,drawTargetParticle, RedT, GreenT, BlueT, TargetCalculator, EzCalc, ProcastCalc, RocketCounter, ezKill,  blinkKey, panel, statusPosX, statusPosY} from "./MenuManager"
import { ArrayExtensions, Color, Game, Hero, ParticlesSDK, RendererSDK, Vector2, Vector3, Utils, Sleeper, GameSleeper } from "wrapper/Imports"
import { Heroes, MouseTarget, MyHero, savespots } from "./Listeners"
import { ProcastCounter, EZKill, GetEZKillDamage, GetComboDamage, GetOneAutoAttackDamage, OnlyRocketCount, Manaonerocket, Manaprocast } from "./Module/Calc"
...
export function Draw() {
	if (active.value&&MyHero.Team !== 1&&Game.IsInGame && MyHero !== undefined)
	{
	...
	if (drawTargetParticle.value) {
		if (targetParticle === undefined && MouseTarget !== undefined) {
			targetParticle = ParticlesSDK.Create("materials/ensage_ui/particles/target.vpcf", ParticleAttachment_t.PATTACH_ABSORIGIN, MouseTarget)
		}
	...
	if (info.length>0 && (TargetCalculator.value||RocketCounter.value)&&Abilities.r.Level>0)//ALL TARGETS DRAW
		...
		info.forEach(hero => {
			...
			if (RocketCounter.value)
				RendererSDK.Text("          x" + OnlyRocketCount(hero).toFixed(0) + " rkts", wts.Add(new Vector2(65,0)),Math.ceil(((MyHero.Mana+MyHero.ManaRegen*OnlyRocketCount(hero)*(Abilities.r.GetSpecialValue("channel_tooltip")+Abilities.r.CastPoint+0.1)) / (Manaonerocket() + Abilities.r.ManaCost))) >= OnlyRocketCount(hero) ? Color.Green : Color.RoyalBlue, "Verdana", 14, FontFlags_t.OUTLINE)
			}
		}
			});
		}
