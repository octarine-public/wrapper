...
import { MyHero, initItemsMap, initAbilityMap, initItemsTargetMap, initHitAndRunMap } from "../Listeners"
import {
...
export function AutoCombo() {
	...
	let enemyLoser = ArrayExtensions.orderBy(
		EntityManager.GetEntitiesByClass(Hero, DOTA_UNIT_TARGET_TEAM.DOTA_UNIT_TARGET_TEAM_ENEMY).filter(x =>
			...
			&& !x.IsMagicImmune
			&& Base.Active(x)
			&& (x.IsStunned || Base.TriggerAutoCombo(x))
		), x => x.Distance2D(MyHero))[0]

	if (enemyLoser === undefined || !Base.Cancel(enemyLoser) || Base.AeonDisc(enemyLoser, false) || (BladeMailCancelCombo.value && enemyLoser.HasBuffByName("modifier_item_blade_mail_reflect"))) {
		return
	}
	...
	if (AbilityArray.some(x => x?.CanBeCasted() && Combo(x, enemyLoser, Abilities, Items, ItemsTarget, AutoComboItems, AutoComboAbility)))
		return
