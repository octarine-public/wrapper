...
function ChasingReady(selector: Menu.ImageSelector, ability: Ability, enemy: Unit) {
...
export function InitHarras() {
	...
	if (!Target.IsMagicImmune || !CheckBuffToReflect()) {
		if (arr_abil.some(abil => !Sleep.Sleeping(abil) && ChasingReady(HarrasAbility, abil, Target))) {
			return
		}
	}
...
function CheckBuffToReflect() {
	...
	BuffForCheckTarget.some(items => {
		if (Target.GetBuffByName(items)) {
			if (items !== "modifier_nyx_assassin_spiked_carapace") {
				return false
			}
			return true
		}
