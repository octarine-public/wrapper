...
import { PickupRune, PickupItem } from "../Orders";
...
let allRunes: C_DOTA_Item_Rune[] = [],
	allRunesParticles = [],
	needItems: C_DOTA_Item_Physical[] = [],
	controllables: C_DOTA_BaseNPC[] = [];

const snatcherMenu = new MenuManager("Snatcher JS");
...
const takeRadius = snatcherMenu.AddSlider("Take radius", 150, 50, 500, "By default - 150, that range dont need rotate unit for take from ground");
...
const statusPosX = drawMenu.AddSlider("Position X (%)", 0, 0, 100);
const statusPosY = drawMenu.AddSlider("Position Y (%)", 75, 0, 100);
...
function onCheckEntity(ent: C_BaseEntity) {
	...
	if (ent instanceof C_DOTA_BaseNPC && stateControllables.value) {
		...
		if (!controllables.includes(ent)
			&& LocalDOTAPlayer !== undefined 
			&& ent.m_iUnitType != 0 && !ent.m_bIsIllusion
