import {  Game, Unit, Entity, Creep, Item, ArrayExtensions, LocalPlayer } from "wrapper/Imports";
...
function GetAllCreepsForMidas(Unit: Unit, Item: Item): Array<Creep> {
	...
	ItemSlot.filter(items => Сompare(items.Name) !== undefined).map(Item => {
		...
		switch (Item.Name) {
			...
			case "item_hand_of_midas":
				...
				} else GetAllCreepsForMidas(unit, Item);
