	...
	Particle.forEach(([particle_id, handle, target, Time, position], i) => {
		...
		if (target === undefined){
			if(handle !== 16169843851719108633n && handle !== 14221266834388661971n)
				RendererSDK.DrawMiniMapIcon("minimap_creep", 500,position,DrawRGBA.Color)
			else if(handle === 14221266834388661971n && !position.Equals(new Vector3(1200,1,1200))){
				RendererSDK.DrawMiniMapIcon("minimap_ping", 700,position,DrawRGBA.Color)
			}
			return;
		}
		...
		RendererSDK.DrawMiniMapIcon(`minimap_heroicon_${Name}`, 900,position,fullColor)
		if(target.IsVisible)
			return
