...
export class TextInput extends Base {
	...
	public static OnWindowSizeChanged(): void {
		...
		TextInput.iconSize.x = ScaleWidth(TextInput.origIconSize.x)
		TextInput.iconSize.y = ScaleHeight(TextInput.origIconSize.y)
		TextInput.iconOffset.x = ScaleWidth(10)
	...
	public get textLeftOffset(): number {
		return TextInput.iconOffset.x + TextInput.iconSize.x + TextInput.inputPadding.x
	}
	...
	public override Render(): void {
		...
		RendererSDK.Image(
			...
			-1,
			TextInput.iconSize,
			isFocused ? undefined : TextInput.placeholderColor
		)
		...
		if (!hasText && !isFocused) {
			...
			const placeholderSize =
				this.GetTextSizeDefault(this.placeholder)
			RendererSDK.FilledRect(
				textPos.Clone().AddScalarY(placeholderSize.y + TextInput.underlineHeight * 2),
				new Vector2(placeholderSize.x, TextInput.underlineHeight),
				TextInput.placeholderColor
			)
		} else {
			...
			const size3 = RendererSDK.GetTextSize(this.text, this.FontName, size, this.FontWeight)

			RendererSDK.Text(
				this.text,
				textPos.Subtract(new Vector2(0, Math.round(size3.y - size / 1.5 + size3.z / 2))),
				undefined,
				this.FontName,
				size,
				this.FontWeight,
