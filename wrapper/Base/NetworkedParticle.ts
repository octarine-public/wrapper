...
export class NetworkedParticle {
	...
	private setParticleData(pathNoEcon: string): void {
		...
		const { target, source, abilityName, attach } = obj
		if (attach !== this.Attach) {
			return
		}
		let caster = this.getCaster(source)
		const ability = this.getAbility(abilityName, caster, source)
		if (caster === undefined) {
	...
	private getTarget({ controlPointEnt, isAttachedTo }: ITargetData): Nullable<Unit> {
		if (isAttachedTo && this.AttachedTo instanceof Unit) {
			return this.AttachedTo
		}
	...
	private getAbility(
		...
		if (Array.isArray(name)) {
			for (let i = name.length - 1; i > -1; i--) {
				const ability = this.getAbility(name[i], caster, {
					isTeamReverted,
					findCaster
				})
		...
		if (findCaster === undefined) {
			return constructor.prototype instanceof Item
				? caster?.GetItemByClass(constructor as Constructor<Item>)
				: caster?.GetAbilityByClass(constructor)
		}
		const abilities = EntityManager.GetEntitiesByClass(constructor).orderBy(
			x => x.Cooldown
		)
		return isTeamReverted
			? abilities.find(x => x.IsEnemy())
			: abilities.find(x => !x.IsEnemy())
	}
}
