...
export class Inventory {
	...
	public get Items(): Item[] {
		...
		if (tp !== undefined) ar.push(tp)
		if (neutral !== undefined) ar.push(neutral)
		return ar
	}
	...
	public GetFreeSlots(
		...
		if (this.Owner.IsValid)
			for (let i = start; i <= end; i++)
				if (this.GetItem(i) === undefined) items.push(i)
		return items
	}
	public HasAnyItem(
		start: DOTAScriptInventorySlot,
		...
		if (this.Owner.IsValid)
			for (let i = start; i < Math.min(end + 1, MAX_ITEMS); i++)
				if (this.GetItem(i) !== undefined) return true
		return false
	}
	public HasFreeSlot(
		start: DOTAScriptInventorySlot,
	...
	public HasFreeSlots(
		...
		if (!this.Owner.IsValid) return false

		start = Math.min(start, MAX_ITEMS)
		end = Math.min(end, MAX_ITEMS)
		let man = 0
		for (let i = start; i <= end; i++) if (this.GetItem(i) === undefined) man++
		return man >= howMany
	}
	...
	public CountItemByOtherPlayer(
		player: Nullable<Player> = LocalPlayer
	): number {
		if (player === undefined) return 0
		return this.TotalItems.reduce((counter, item) => {
			if (item !== undefined && item.PurchaserID === player.PlayerID) counter++
			return counter
		}, 0)
	}
	public GetItemByName(
		...
		if (!this.Owner.IsValid) return undefined
		const items = includeBackpack
			? [...this.Items, ...this.Backpack]
			: this.Items
	...
	public GetItemsByNames(
		names: string[],
		includeBackpack: boolean = false
	): Item[] {
		if (!this.Owner.IsValid) return []
		const items = includeBackpack
			? [...this.Items, ...this.Backpack]
			: this.Items
	...
	public GetItemsByClasses<T extends Item>(
		...
		return items.filter(item =>
			classes.some(class_ => item instanceof class_)
		) as T[]
	}
}
