...
export class modifier_brewmaster_drunken_brawler_passive extends Modifier {
	...
	private get multiplier(): number {
		...
		const hasMul =
			owner.HasBuffByName("modifier_brewmaster_drunken_brawler_brewed_up") ?? false
		return !hasMul ? 1 : Math.max(1 + this.cachedMultiplier, 1)
	}
	public PostDataUpdate(): void {
		const ability = this.Ability
		if (!(ability instanceof brewmaster_drunken_brawler)) {
			this.cachedMS = this.cachedMR = this.cachedAR = this.cachedAS = 0
			return
		}
	...
	protected UpdateSpecialValues(): void {
		...
		this.cachedMultiplier = this.GetSpecialValue("brewed_up_bonus", name) / 100
	}
}
