...
export class npc_dota_hero_pudge extends Hero {
	public CalculateActivityModifiers(
		...
		if (hook !== undefined) {
			if (hook.ConsecutiveHits >= 3) ar.push("hook_streak_large")
			else if (hook.ConsecutiveHits >= 2) ar.push("hook_streak_medium")
			else if (hook.ConsecutiveHits >= 1) ar.push("hook_streak_small")
		}
	}
}
