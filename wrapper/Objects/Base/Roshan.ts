...
EventsSDK.on("ParticleCreated", par => {
	if (par.PathNoEcon !== "particles/neutral_fx/roshan_spawn.vpcf") return
	Roshan.Instance = par.AttachedTo
	if (GameRules === undefined) return
...
EventsSDK.on("LifeStateChanged", ent => {
	if (Roshan.Instance === ent) Roshan.HP = 0
})
