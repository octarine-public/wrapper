...
export default new (class Workers {
	...
	public CallRPCEndPoint(name: string, data: WorkerIPCType, high_priority = false): Promise<WorkerIPCType> {
		...
		return new Promise<WorkerIPCType>(async (resolve, reject) => {
			const worker = orderBy(this.workers, ar => ar[2].length) // pick the least used worker
				.find(ar => ar[3] && (!high_priority || ar[2].length === 0))
			if (worker !== undefined) {
	...
	private AddWorkerWithOpts(opts: WorkerOptions): void {
