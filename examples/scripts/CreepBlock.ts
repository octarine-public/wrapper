...
var config = {
	...
	lane_creeps: C_DOTA_BaseNPC_Creep_Lane[] = [],
	towers: C_DOTA_BaseNPC_Tower[] = [],
	enabled = false

Events.RegisterCallback("onPrepareUnitOrders", (args: CUnitOrder) =>
	!args.from_script && enabled
	? args.order_type !== dotaunitorder_t.DOTA_UNIT_ORDER_MOVE_TO_POSITION
		&& args.order_type !== dotaunitorder_t.DOTA_UNIT_ORDER_MOVE_TO_DIRECTION
	: true
)
Events.RegisterCallback("onUpdate", () => {
	...
	if(creeps.length === 0)
		return
	var creepsMovePositionSum = creeps.map(creep => creep.InFront(300)).map(vec => [vec.x, vec.z, vec.y]).reduce((sum, vec) => sum ? [sum[0] + vec[0], sum[1] + vec[1], sum[2] + vec[2]] : vec),
		...
		tower = towers.filter(ent => MyEnt.IsInRange(ent, 120)),
		need_select = false//LocalDOTAPlayer.m_nSelectedUnits[0] !== MyEnt
...
Events.RegisterCallback("onNPCCreated", (npc: C_DOTA_BaseNPC) => {
	if (npc.m_bIsLaneCreep && !npc.IsEnemy(LocalDOTAPlayer))
		lane_creeps.push(<C_DOTA_BaseNPC_Creep>npc)
	if (npc.m_bIsTower)
		towers.push(<C_DOTA_BaseNPC_Tower>npc)
})
Events.RegisterCallback("onEntityDestroyed", (ent: C_BaseEntity) => {
	if (!ent.m_bIsDOTANPC)
		return
	if ((<C_DOTA_BaseNPC>ent).m_bIsLaneCreep && !ent.IsEnemy(LocalDOTAPlayer))
		lane_creeps.splice(lane_creeps.indexOf(<C_DOTA_BaseNPC_Creep_Lane>ent), 1)
	if ((<C_DOTA_BaseNPC>ent).m_bIsTower)
		towers.splice(towers.indexOf(<C_DOTA_BaseNPC_Tower>ent), 1)
})
